...
## 2a. Borough-Level Shooting Counts (2006–2024)

This subsection summarizes the total number of shooting incidents for each borough using the historic dataset from 2006 to 2024.
...
dat1_boro |>
  ...
  kable(
    ...
    align = c("l", "r"),
    caption = "Shooting Incident Counts by Borough (2006 to 2024)"
  ) |>
  kable_styling(full_width = FALSE, position = "center")
```

* **Comments**
Brooklyn and the Bronx have the highest total shooting counts in the historic period, together accounting for the majority of incidents citywide. Queens and Manhattan show moderate levels, while Staten Island has the fewest shootings. This distribution reflects substantial variation in shooting activity across boroughs.
...
dat1_location |>
  ...
  kable(
    ...
    align = c("l", "r"),
    caption = "Shooting Incidents by Location Type (2006 to 2024)"
  ) |>
  kable_styling(full_width = FALSE, position = "center")
```
...
<<<<<<< HEAD
This section presents interactive maps that visually display geographic differences in shooting incidents across boroughs. Each shooting incident is shown as a point marker on the map, with clustering enabled to improve performance and readability.
=======
This section presents interactive maps that visually display geographic differences in shooting incidents across boroughs. Each shooting incident is shown as a point marker on the map.
>>>>>>> b9442a89d7f650fdc46fa7adda3e5ac6ee8623d2

## 3a. Map of Total Shootings by Borough (2006–2024)

This map shows individual shooting incidents as points overlaid on borough boundaries for the historic period.
...
dat1_boro = boro_to_county(dat1_boro)
nyc_boro_map1 = nyc_boro |>
  left_join(dat1_boro, by = c("NAME" = "county_name"))
```

```{r map-historic-total}
mapview(
  nyc_boro_map1,
  zcol = "n_total",
...
mapview(
  ...
  col.regions = "red",
<<<<<<< HEAD
  layer.name = "Shootings (2006-2024)",
...
* **Comments**
<<<<<<< HEAD
Each red point represents an individual shooting incident from 2006 to 2024. Points are clustered for improved visualization; click on clusters to zoom in and see individual incidents. The clustering of points in Brooklyn and the Bronx is clearly visible, showing the geographic concentration of gun violence in these boroughs. The borough shading indicates total counts, while points show exact incident locations.
=======
Each red point represents an individual shooting incident from 2006 to 2024. The clustering of points in Brooklyn and the Bronx is clearly visible, showing the geographic concentration of gun violence in these boroughs. The borough shading indicates total counts, while points show exact incident locations.
>>>>>>> b9442a89d7f650fdc46fa7adda3e5ac6ee8623d2


## 3b. Map of Total Shootings by Borough (All Years)

This map incorporates all years of data including 2025, showing each shooting as a point marker.

```{r map-all-setup, results='hide'}
...
* **Comments**
The geographic distribution of shootings remains stable when 2025 data is included. The point density in Brooklyn and the Bronx continues to dominate the map. Points in Manhattan tend to cluster in specific neighborhoods, while Queens shows a more dispersed pattern.


## 3c. Map of Inside Shootings by Borough (All Years)

This map displays inside shootings as point markers, allowing visualization of where indoor incidents occur.

```{r map-inside-setup, results='hide'}
dat_inside_boro = dat_all |>
...
nyc_boro_map_inside = nyc_boro |>
  left_join(dat_inside_boro, by = c("NAME" = "county_name"))
...
mapview(
  nyc_boro_map_inside,
  zcol = "n_inside",
  layer.name = "Borough Totals (Inside)",
  alpha.regions = 0.3
) + 
mapview(
  dat_inside_sf,
...
* **Comments**
Blue points represent shootings that occurred inside buildings. The pattern shows inside shootings are distributed across the same high-incident areas as overall shootings, with notable concentrations in Brooklyn and the Bronx. Indoor incidents appear in residential and commercial areas throughout the city.


## 3d. Map of Outside Shootings by Borough (All Years)

This map displays outside shootings as point markers, showing the distribution of street-level gun violence.

```{r map-outside-setup, results='hide'}
dat_outside_boro = dat_all |>
...
nyc_boro_map_outside = nyc_boro |>
  left_join(dat_outside_boro, by = c("NAME" = "county_name"))
...
```{r map-outside}
mapview(
  nyc_boro_map_outside,
  zcol = "n_outside",
  layer.name = "Borough Totals (Outside)",
  alpha.regions = 0.3
) + 
mapview(
  dat_outside_sf,
...
* **Comments**
Orange points represent shootings that occurred outside (street-level). The clustering pattern is more pronounced for outside shootings, reflecting the concentration of street violence in specific neighborhoods. Brooklyn and the Bronx show the highest density of outdoor shooting incidents.


## 3e. Map Comparing Inside vs Outside Shootings (All Years)
...
3. **Geographic clustering:** The point maps reveal that shootings cluster in specific neighborhoods within each borough, with the highest density in parts of Brooklyn and the Bronx. Both inside and outside shootings follow similar geographic patterns.
